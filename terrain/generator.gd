...
#@export var tree_group: LookupGroup2:
	#set(value):
		#tree_group = value
...
var block_lib: BlockLibrary = preload("res://data/blocks/block_library.tres")

var dirt_index: int = block_lib.get_block(&"dirt").index
var grass_index: int = block_lib.get_block(&"grassed_dirt").index
			...
			for x: int in range(min_pos.x, max_pos.x):
				...
				for y: int in range(min_pos.y, max_pos.y):
					...
					if y < terrain_height:
						...
						voxel_tool.set_voxel(position, 0) # air
